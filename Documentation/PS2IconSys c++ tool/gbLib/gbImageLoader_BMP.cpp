...
#ifdef WIN32	// Issun was here: this was originally 'ifndef' but we got "packed undeclared identifier".
...
#	include <poppack.h>
	...
	static void FlipImageVertical(BITMAPINFOHEADER const& iheader, unsigned char* data)
	{
		...
		for (int y = 0; y < iheader.biHeight / 2; y++)
		{
			...
			{
				for (int q = 0; q < p; q++)
				{			//loop over different color channels
...
#	pragma warning(disable: 4100)
		...
		unsigned char* data, unsigned int** palette, unsigned int file_offset)
	{
		...
		case 3:						//uncompressed rgb with color masks (32 bits)
			...
			for (int i = 0; i < iheader.biWidth * iheader.biHeight; i++)
			{		//apply color masks
