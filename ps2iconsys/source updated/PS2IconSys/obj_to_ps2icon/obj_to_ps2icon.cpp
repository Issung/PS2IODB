 ...
 */
#include <iostream>
#include "../src/ps2_ps2icon.hpp"
#include "../src/obj_loader.hpp"
#include "../gbLib/gbException.hpp"
#include "../gbLib/gbImageLoader.hpp"
...
OBJ_FileLoader* LoadOBJFile()
{
	...
	{
		std::cout << "Invalid mesh index. Index given: " << obj_mesh_index << "; Maximum allowed for \"" << obj_input_file << "\": " << (ret->GetNMeshes() - 1) << std::endl;
		exit(1);
	}
...
void WriteOutputFile(OBJ_FileLoader const* obj_file, GhulbusUtil::gbImageLoader* img_loader)
{
	...
	if (verbose_output)
	{
		std::cout << " * Copying geometry data from \"" << obj_input_file << "\": Mesh #" << obj_mesh_index << " - " << tmp->GetName() << "...";
	}
...
int main(int argc, char* argv[])
{
	...
	if ((!list_obj_file) && (!ps2_output_file)) 
	{
		ps2_output_file = "default.icn"; 
	}
